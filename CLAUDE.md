...
### 트랙 A: 섹터로테이션 (월간)
- **주기**: 월 1회 (월초 08:00 스크리닝 → RSI 대기 → 08:50 매수)
- **종목풀**: 7개 고정 섹터 ETF
...
- **복합 모멘텀 점수** = (1M수익률 × 0.6) + (3M수익률 × 0.4)
  ...
  - 3M 데이터 없으면 1M만 사용
- **RSI(3) 진입 필터**: 1위 ETF 선정 후 RSI(3) < 50 눌림목 확인 후 진입
  - RSI(3) < 50 → BUY (당일 08:50 매수)
  - RSI(3) >= 50 → WAIT (내일 08:00 재확인)
  - 월말까지 미달 → PASS (해당 월 패스, 현금 유지)
  - RSI 계산: Wilder's Smoothing 방식, 20일치 데이터 사용
- **매매규칙**: 200만원 지정가 매수 (전일 종가 +1%), 익절 +7%, 손절 -5% (지정가 기준), 월말 미도달 시 종가 매도
- **스크리닝 시간 제한**: 08:00~08:45 가능 (전일 종가 확정 기준)

### 트랙 B: 단기스윙 (주간)
- **주기**: 매일(월~금) 08:00 스크리닝, 주 1회 매수 (08:50 시장가)
...
### 트랙 C: 볼린저밴드 %B (매일)
- **주기**: 매일 (08:00 스크리닝 → 08:50 매수)
- **종목풀**: 8개 섹터 ETF (섹터 7개 + KODEX 코스닥150)
- **매수 조건**: %B < 0.10 AND 거래량 ≥ 20일 평균의 1.5배
  - 복수 신호 시 %B 가장 낮은 종목 1개
  - 매수 타이밍: 당일 08:50 지정가 (전일 종가 +1%)
- **청산 조건**: 장중 MA20 돌파 시 즉시 시장가 매도, 익절 %B ≥ 0.5 (08:00 이후 확인 → 08:50 매도), 손절 -5% (매수 당일 지정가 등록)
