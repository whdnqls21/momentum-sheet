  ...
  try {
    ...
    if (screenDate) {
      const res = await fetch(
        `${process.env.SUPABASE_URL}/rest/v1/screening_history?strategy=eq.swing&screen_date=eq.${screenDate}&select=*`,
        {
          headers: {
    ...
    // 이력 목록 조회 (최근 20건, screen_date 내림차순)
    const { data, error } = await supabase
      .from('screening_history')
      .select('screen_date, week_label, selected_ticker, selected_name, created_at')
      .eq('strategy', 'swing')
    ...
    console.error('[Swing History] 에러:', (err as Error).message);
